...
<span id="addset0">
<?
$ms=mysql_query("SELECT * FROM  `set_models` WHERE id_cat='0' ORDER BY weight DESC, idd ASC");
...
<?
$ms=mysql_query("SELECT * FROM  `set_models` WHERE id_cat='1' ORDER BY weight DESC, idd ASC");
...
<?
$ms=mysql_query("SELECT * FROM  `set_models` WHERE id_cat='2' ORDER BY weight DESC, idd ASC");
while($mset=mysql_fetch_array($ms)){ echo '<tr class="id_'.$mset['id'].'"><td style="padding-left: 30px;">'.$mset['name'].'</td><td style="text-align: center;"><!--i class="icon-remove del_set" rel="'.$mset['id'].'" ></i--><i class="icon-pencil edit_set" rel="'.$mset['id'].'"></i></td></tr>';}
?>
</table>
...
?>
    ...
    <div class="controls">
<select id="section2" name="weight">
...
<option value="1">Комерческий</option>
  ...
  <div class="fileupload fileupload-new" data-provides="fileupload" style="float: left;margin-left: 5px;">
    ...
    <div class="controls">
      <input type="text" id="inputt"  name="t" value="<?if(isset($_POST['t'])){echo $_POST['t'];}?>" class="input-xxlarge tblock" placeholder="Title" maxlength="70" rel="4" />
  ...
  <div class="control-group  <?echo $error_des;?>">
    ...
    <div class="controls">
      <input type="text" id="inputd" name="d" value="<?if(isset($_POST['d'])){echo $_POST['d'];}?>" class="input-xxlarge tblock" placeholder="description" maxlength="160" rel="6" />
      <span id="span_text6"></span>
    </div>
  </div>

<div class="control-group <?echo $visible;?>">
    <label class="control-label" for="small-1">Visible</label>
    <div class="controls">
    <input id="small-1" name="visible" value="1" type="checkbox" <?if((isset($_POST['visible']) && $_POST['visible'] == 1) || !isset($_POST['visible']) ){echo 'checked="checked"';}?>>
    </div>
    </div>
...
<option value="1">Комерческий</option>
  ...
  <div class="fileupload fileupload-new" data-provides="fileupload" style="float: left;margin-left: 5px;">
    ...
    <label class="control-label" for="inputt">Title <i class="icon-question-sign po" style="margin: 7px 0px; 0 0" onclick="$('.bottom-left').notify({message: {text: 'Определяет заголовок документа.'}}).show();"></i></label>
    <div class="controls">
      <input type="text" id="inputt"  name="t" value="<?if(isset($_POST['t']) and $_POST['t']!=''){echo $_POST['t'];}else{echo $model['t'];}?>" class="input-xxlarge tblock" placeholder="Title" maxlength="70" rel="4" />
      <span id="span_text4"></span>
    </div>
  </div>
...
?>
 <?echo'<a href="?models&view='.$_GET['view'].'&galery&meta" class="pull-right"> Редактировать мета-теги</a><br/>';?>
<?        if(isset($_GET['meta'])){
        $p_mod=mysql_fetch_array(mysql_query("SELECT * FROM  `models` WHERE `id`='".$_GET['view']."' "));
        if(!empty($_POST)){
        include_once "../component/translit.php";
        $t=mysql_real_escape_string($_POST['gal_t']);
        $d=mysql_real_escape_string($_POST['gal_d']);
        $k=mysql_real_escape_string($_POST['gal_k']);
        $add=mysql_query("UPDATE `models` SET  `gal_k`='".$k."', `gal_d`='".$d."', `gal_t`='".$t."' WHERE `id`='".$_GET['view']."'");
        echo "<html> <head> <meta HTTP-EQUIV='Refresh' CONTENT='0; URL=./?models&view=".$_GET['view']."&galery'></head><body>"; }
        echo '<tr><td><form class="form-horizontal" method="post" action="" enctype="multipart/form-data"><fieldset><legend>Редактировать мета-теги</legend> <div class="control-group"><div class="control-group "><label class="control-label" for="inputt">Title </label><div class="controls"><input type="text" id="inputt"  name="gal_t" value="'.$p_mod['gal_t'].'" class="input-xxlarge tblock" placeholder="Title" maxlength="70" rel="4" /><span id="span_text4"></span></div></div><div class="control-group "><label class="control-label" for="inputk">Ключевые слова </label><div class="controls"><input type="text" id="inputk"  name="gal_k" value="'.$p_mod['gal_k'].'" class="input-xxlarge tblock" placeholder="Ключевые слова" maxlength="500" rel="5" /><span id="span_text5"></span></div></div><div class="control-group  "><label class="control-label" for="inputd">description</label><div class="controls"><input type="text" id="inputd" name="gal_d" value="'.$p_mod['gal_d'].'" class="input-xxlarge tblock" placeholder="description" maxlength="500" rel="6" /><span id="span_text6"></span></div></div><div class="control-group"> <div class="controls"><input type="submit" value="Сохранить" class="btn btn-primary" /> </div></div></fieldset></form></td></tr>';
}?>
...
<?}
elseif(isset($_GET['overview'])){echo 'Обзор  <a href="?models&view='.$_GET['view'].'&overview&add" class="pull-right">Добавить Обзор</a><br/> <a href="?models&view='.$_GET['view'].'&overview&meta" class="pull-right"> Редактировать мета-теги</a><br/>';}
elseif(isset($_GET['features'])){echo 'Комплектации <a href="?models&view='.$_GET['view'].'&features&add" class="pull-right">Добавить Комплектацию</a><br/> <a href="?models&view='.$_GET['view'].'&features&meta" class="pull-right"> Редактировать мета-теги</a><br/>';}
elseif(isset($_GET['catalog'])){echo 'Каталоги <a href="?models&view='.$_GET['view'].'&catalog&add" class="pull-right">Добавить новый</a><br/> <a href="?models&view='.$_GET['view'].'&catalog&meta" class="pull-right"> Редактировать мета-теги</a><br/>';}

elseif(isset($_GET['special'])){echo 'Спецпредложение <a href="?models&view='.$_GET['view'].'&special&add" class="pull-right">Добавить Спецпредложение</a><br/> <a href="?models&view='.$_GET['view'].'&special&meta" class="pull-right"> Редактировать мета-теги</a><br/>';}
elseif(isset($_GET['sale_cars'])){echo 'Акционные автомобили <a href="?models&view='.$_GET['view'].'&sale_cars&add" class="pull-right">Добавить акционный автомобиль</a>';}
else{?>
...
if(isset($_GET['galery'])){
    ...
    while($catalog=mysql_fetch_array($ct)){ 
        ...
        if(!empty($_POST)){
        include_once "../component/translit.php";
        $t=mysql_real_escape_string($_POST['comp_t']);
        $d=mysql_real_escape_string($_POST['comp_d']);
        $k=mysql_real_escape_string($_POST['comp_k']);
        $add=mysql_query("UPDATE `models` SET  `comp_k`='".$k."', `comp_d`='".$d."', `comp_t`='".$t."' WHERE `id`='".$_GET['view']."'");
...
?>
    ...
    <legend>Добавить Комплектацию</legend>
        ...
        <div class="controls">
        <textarea type="text" id="inputdk"  name="dvig[]"  class="input-xlarge" placeholder="двигатель и трансмиссия" /><i class="icon-plus dtplus" reg="1"></i>-->
    ...
    <legend>Установить Комплектацию</legend>
        ...
        while($teh=mysql_fetch_array($tf)){
            ...
            echo'<div class="control-group" id="controls'.$teh['id'].'">
                ...
                <i style="margin: 3px 0 0 5px;" rel="'.$teh['id'].'" class="icon-remove tool remove_comp" data-title="Удалить вместе с содержимым"></i>
                <div class="cl"></div>
...
if(isset($_GET['meta'])){
        ...
        if(!empty($_POST)){
        include_once "../component/translit.php";
        $t=mysql_real_escape_string($_POST['cat_t']);
        $d=mysql_real_escape_string($_POST['cat_d']);
        $k=mysql_real_escape_string($_POST['cat_k']);
        $add=mysql_query("UPDATE `models` SET  `cat_k`='".$k."', `cat_d`='".$d."', `cat_t`='".$t."' WHERE `id`='".$_GET['view']."'");
        echo "<html> <head> <meta HTTP-EQUIV='Refresh' CONTENT='0; URL=./?models&view=".$_GET['view']."&catalog'></head><body>"; }
    ...
    if(isset($_GET['add'])){
        ...
        echo "<html> <head> <meta HTTP-EQUIV='Refresh' CONTENT='0; URL=./?models&view=".$_GET['view']."&overview'></head><body>"; }
        echo '<tr><td><form class="form-horizontal" method="post" action="" enctype="multipart/form-data"><fieldset><legend>Редактировать Обзор</legend> <div class="control-group"><label class="control-label" for="inputname">Название</label><div class="controls"><input type="text" id="inputname"  name="name" value="'.$p_mod['name'].'" class="input-xxlarge tblock" placeholder="Название" maxlength="250" rel="1" /><span id="span_text1"></span></div></div> <div class="control-group"><label class="control-label" for="inputname3">URL</label><div class="controls"><input type="text" id="inputname3" name="link" value="'.$p_mod['link'].'" class="input-xxlarge tblock" placeholder="URL Адрес" maxlength="250" rel="2" /><span id="span_text2"></span></div></div><div class="control-group"><label class="control-label" for="textarea1">Описание</label><div class="controls"><textarea name="text" id="textarea1" placeholder="Описание">'.$p_mod['text'].'</textarea></div></div><div class="control-group "><label class="control-label" for="inputt">Title </label><div class="controls"><input type="text" id="inputt"  name="t" value="'.$p_mod['t'].'" class="input-xxlarge tblock" placeholder="Title" maxlength="70" rel="4" /><span id="span_text4"></span></div></div><div class="control-group "><label class="control-label" for="inputk">Ключевые слова </label><div class="controls"><input type="text" id="inputk"  name="k" value="'.$p_mod['k'].'" class="input-xxlarge tblock" placeholder="Ключевые слова" maxlength="500" rel="5" /><span id="span_text5"></span></div></div><div class="control-group  "><label class="control-label" for="inputd">description</label><div class="controls"><input type="text" id="inputd" name="d" value="'.$p_mod['d'].'" class="input-xxlarge tblock" placeholder="description" maxlength="160" rel="6" /><span id="span_text6"></span></div></div><div class="control-group"> <div class="controls"><input type="submit" value="Сохранить" class="btn btn-primary" /> </div></div></fieldset></form></td></tr>';
}elseif(isset($_GET['meta'])){
        $p_mod=mysql_fetch_array(mysql_query("SELECT * FROM  `models` WHERE `id`='".$_GET['view']."' "));
        if(!empty($_POST)){
        include_once "../component/translit.php";
        $t=mysql_real_escape_string($_POST['overview_t']);
        $d=mysql_real_escape_string($_POST['overview_d']);
        $k=mysql_real_escape_string($_POST['overview_k']);
        $add=mysql_query("UPDATE `models` SET  `overview_k`='".$k."', `overview_d`='".$d."', `overview_t`='".$t."' WHERE `id`='".$_GET['view']."'");
        echo "<html> <head> <meta HTTP-EQUIV='Refresh' CONTENT='0; URL=./?models&view=".$_GET['view']."&overview'></head><body>"; }
        echo '<tr><td><form class="form-horizontal" method="post" action="" enctype="multipart/form-data"><fieldset><legend>Редактировать мета-теги</legend> <div class="control-group"><div class="control-group "><label class="control-label" for="inputt">Title </label><div class="controls"><input type="text" id="inputt"  name="overview_t" value="'.$p_mod['overview_t'].'" class="input-xxlarge tblock" placeholder="Title" maxlength="70" rel="4" /><span id="span_text4"></span></div></div><div class="control-group "><label class="control-label" for="inputk">Ключевые слова </label><div class="controls"><input type="text" id="inputk"  name="overview_k" value="'.$p_mod['overview_k'].'" class="input-xxlarge tblock" placeholder="Ключевые слова" maxlength="500" rel="5" /><span id="span_text5"></span></div></div><div class="control-group  "><label class="control-label" for="inputd">description</label><div class="controls"><input type="text" id="inputd" name="overview_d" value="'.$p_mod['overview_d'].'" class="input-xxlarge tblock" placeholder="description" maxlength="160" rel="6" /><span id="span_text6"></span></div></div><div class="control-group"> <div class="controls"><input type="submit" value="Сохранить" class="btn btn-primary" /> </div></div></fieldset></form></td></tr>';
    ...
    if(isset($_GET['add'])){
        ...
        $d=mysql_real_escape_string($_POST['d']);
        $k=mysql_real_escape_string($_POST['k']);
        $add=mysql_query("INSERT INTO `model_page` SET `tip`='2',`model`='".$_GET['view']."', `name`='".$name."',`link`='".$link."', `k`='".$k."', `d`='".$d."', `t`='".$t."', `text`='".$text."',`date`=NOW()");
        $id=mysql_insert_id();
        $add=mysql_query("UPDATE `model_page` SET `idd`='".$id."' WHERE `id`='".$id."'");
        echo "<html> <head> <meta HTTP-EQUIV='Refresh' CONTENT='0; URL=./?models&view=".$_GET['view']."&sale_cars'></head><body>"; }
        echo '<tr><td><form class="form-horizontal" method="post" action="" enctype="multipart/form-data"><fieldset><legend>Добавить акционный автомобиль</legend><div class="control-group"><label class="control-label" for="inputname">Название</label><div class="controls"><input type="text" id="inputname"  name="name" value="" class="input-xxlarge tblock" placeholder="Название" maxlength="250" rel="1" /><span id="span_text1"></span></div></div><div class="control-group"><label class="control-label" for="inputname3">URL</label><div class="controls"><input type="text" id="inputname3" name="link" value="" class="input-xxlarge tblock" placeholder="URL Адрес" maxlength="250" rel="2" /><span id="span_text2"></span></div></div><div class="control-group"><label class="control-label" for="textarea1">Описание</label><div class="controls"><textarea name="text" id="textarea1" placeholder="Описание"></textarea></div></div><div class="control-group "><label class="control-label" for="inputt">Title </label><div class="controls"><input type="text" id="inputt"  name="t" value="" class="input-xxlarge tblock" placeholder="Title" maxlength="70" rel="4" /><span id="span_text4"></span></div></div><div class="control-group "><label class="control-label" for="inputk">Ключевые слова </label><div class="controls"><input type="text" id="inputk"  name="k" value="" class="input-xxlarge tblock" placeholder="Ключевые слова" maxlength="500" rel="5" /><span id="span_text5"></span></div></div><div class="control-group  "><label class="control-label" for="inputd">description</label><div class="controls"><input type="text" id="inputd" name="d" value="" class="input-xxlarge tblock" placeholder="description" maxlength="160" rel="6" /><span id="span_text6"></span></div></div><div class="control-group"> <div class="controls"><input type="submit" value="Сохранить" class="btn btn-primary" /> </div></div></fieldset></form></td></tr>';
    }elseif(isset($_GET['edit'])){
        $p_mod=mysql_fetch_array(mysql_query("SELECT * FROM  `model_page` WHERE `id`='".$_GET['edit']."' AND `tip`='2' LIMIT 1"));
        if(!empty($_POST)){
        include_once "../component/translit.php";
        $link=translitIt($_POST['link']);
        $name=mysql_real_escape_string($_POST['name']);
        ...
        $add=mysql_query("UPDATE  `model_page` SET  `name`='".$name."',`link`='".$link."', `k`='".$k."', `d`='".$d."', `t`='".$t."', `text`='".$text."' WHERE `id`='".$_GET['edit']."'");
        echo "<html> <head> <meta HTTP-EQUIV='Refresh' CONTENT='0; URL=./?models&view=".$_GET['view']."&sale_cars'></head><body>"; }
        echo '<tr><td><form class="form-horizontal" method="post" action="" enctype="multipart/form-data"><fieldset><legend>Редактировать акционный автомобиль</legend> <div class="control-group"><label class="control-label" for="inputname">Название</label><div class="controls"><input type="text" id="inputname"  name="name" value="'.$p_mod['name'].'" class="input-xxlarge tblock" placeholder="Название" maxlength="250" rel="1" /><span id="span_text1"></span></div></div> <div class="control-group"><label class="control-label" for="inputname3">URL</label><div class="controls"><input type="text" id="inputname3" name="link" value="'.$p_mod['link'].'" class="input-xxlarge tblock" placeholder="URL Адрес" maxlength="250" rel="2" /><span id="span_text2"></span></div></div><div class="control-group"><label class="control-label" for="textarea1">Описание</label><div class="controls"><textarea name="text" id="textarea1" placeholder="Описание">'.$p_mod['text'].'</textarea></div></div><div class="control-group "><label class="control-label" for="inputt">Title </label><div class="controls"><input type="text" id="inputt"  name="t" value="'.$p_mod['t'].'" class="input-xxlarge tblock" placeholder="Title" maxlength="70" rel="4" /><span id="span_text4"></span></div></div><div class="control-group "><label class="control-label" for="inputk">Ключевые слова </label><div class="controls"><input type="text" id="inputk"  name="k" value="'.$p_mod['k'].'" class="input-xxlarge tblock" placeholder="Ключевые слова" maxlength="500" rel="5" /><span id="span_text5"></span></div></div><div class="control-group  "><label class="control-label" for="inputd">description</label><div class="controls"><input type="text" id="inputd" name="d" value="'.$p_mod['d'].'" class="input-xxlarge tblock" placeholder="description" maxlength="160" rel="6" /><span id="span_text6"></span></div></div><div class="control-group"> <div class="controls"><input type="submit" value="Сохранить" class="btn btn-primary" /> </div></div></fieldset></form></td></tr>';
    }else{$p=mysql_query("SELECT * FROM  `model_page` WHERE `model`='".$_GET['view']."' AND `tip`='2' ORDER BY idd DESC");while($page_mod=mysql_fetch_array($p)){ echo '<tr id="tr'.$page_mod['id'].'"><td>'.$page_mod['name'].' <i class="icon-remove tool pull-right del_m_p" rel="'.$page_mod['id'].'" data-original-title="Удалить"></i> <a href="?models&view='.$_GET['view'].'&sale_cars&edit='.$page_mod['id'].'"><i class="icon-pencil tool pull-right" data-original-title="Редактировать"></i></a></td></tr>';}}
}
elseif(isset($_GET['special'])){
    if(isset($_GET['meta'])){
        ...
        if(!empty($_POST)){
            include_once "../component/translit.php";
            $t=mysql_real_escape_string($_POST['special_t']);
            $d=mysql_real_escape_string($_POST['special_d']);
            $k=mysql_real_escape_string($_POST['special_k']);
            $add=mysql_query("UPDATE `models` SET  `special_k`='".$k."', `special_d`='".$d."', `special_t`='".$t."' WHERE `id`='".$_GET['view']."'");
            echo "<html> <head> <meta HTTP-EQUIV='Refresh' CONTENT='0; URL=./?models&view=".$_GET['view']."&special'></head><body>"; }
        echo '<tr><td><form class="form-horizontal" method="post" action="" enctype="multipart/form-data"><fieldset><legend>Редактировать мета-теги</legend> <div class="control-group"><div class="control-group "><label class="control-label" for="inputt">Title </label><div class="controls"><input type="text" id="inputt"  name="special_t" value="'.$p_mod['special_t'].'" class="input-xxlarge tblock" placeholder="Title" maxlength="70" rel="4" /><span id="span_text4"></span></div></div><div class="control-group "><label class="control-label" for="inputk">Ключевые слова </label><div class="controls"><input type="text" id="inputk"  name="special_k" value="'.$p_mod['special_k'].'" class="input-xxlarge tblock" placeholder="Ключевые слова" maxlength="150" rel="5" /><span id="span_text5"></span></div></div><div class="control-group  "><label class="control-label" for="inputd">description</label><div class="controls"><input type="text" id="inputd" name="special_d" value="'.$p_mod['special_d'].'" class="input-xxlarge tblock" placeholder="description" maxlength="500" rel="6" /><span id="span_text6"></span></div></div><div class="control-group"> <div class="controls"><input type="submit" value="Сохранить" class="btn btn-primary" /> </div></div></fieldset></form></td></tr>';
    }
    if(isset($_GET['add'])){
        if(!empty($_POST)){
            include_once "../component/translit.php";
...
<?if(isset($_GET['arov_down'])){
                $dow=mysql_query("SELECT * FROM  `models` WHERE idd>$_GET[arov_down]  ORDER BY idd ASC   LIMIT 1");
            ...
            if(isset($_GET['arov_up'])){
                $dow=mysql_query("SELECT * FROM  `models` WHERE idd<$_GET[arov_up] ORDER BY idd DESC  LIMIT 1");
                $down=mysql_fetch_array($dow);
                $onm1=$down['idd'];
                $onm2=$_GET['arov_up'];
                $add1=mysql_query("UPDATE `models` SET idd='0'  WHERE idd='$onm1'");
...
$md=mysql_query("SELECT * FROM  `models` WHERE WEIGHT='0' ORDER BY idd DESC");
while($model=mysql_fetch_array($md)){
    ...
    <a href="?models&view='.$model['id'].'"><i class="icon-share"></i></a>';
...
$md=mysql_query("SELECT * FROM  `models` WHERE WEIGHT='1' ORDER BY idd DESC");
while($model=mysql_fetch_array($md)){
    echo '<tr><td style="width: 120px;"><img src="../photo/model/'.$model['link'].'0.png"/></td><td>'.$model['name'].'<p><b>Цена:</b> '.$model['cena'].'</p></td>
    <td style="width: 120px;"><a href="?models&edit_m='.$model['id'].'"><i class="icon-pencil"></i></a>
    <a href="?models&view='.$model['id'].'"><i class="icon-share"></i></a>';
