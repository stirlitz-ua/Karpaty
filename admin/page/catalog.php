...
if($_GET['catalog']!=''){
switch($_GET['catalog']){
    case('accessories'): $hed="Каталог";$id_menu='5'; break;

}
if($_GET['catalog']=='vitrina'){
  ?>
  ...
  function resizeIframe(obj) {
    ...
    <?
    echo '<iframe src="/accessories/adm.php" width="100%"  scrolling="no" frameborder="0" onload="javascript:resizeIframe(this);"></iframe>';
}
if($hed!=''){
?>
<div class="container">
...
//если все впорядке Добавляем в базу
        ...
        if($ename=='' and  $epname=='' and $elink=='' and $error_title=='' and $error_des=='' and $error_key=='' and $error_photo==''){
            ...
            $key=($_POST['k']);
            $text=($_POST['text']);
            $add=mysql_query("INSERT INTO `accessories` SET `menu`='".$id_menu."',`type`='".$_POST['type']."', `nmenu`='".$mname."',`link`='".$link."', `pmenu`='".$pmenu."', `k`='".$key."', `d`='".$des."', `t`='".$title."', `text`='".$text."',`date`=NOW()");
            $id=mysql_insert_id();
            $add=mysql_query("UPDATE `accessories` SET `idd`='".$id."' WHERE `id`='".$id."'");
            ...
            echo "<html> <head> <meta HTTP-EQUIV='Refresh' CONTENT='0; URL=./?catalog=".$_GET['catalog']."'></head><body>";
        }}
    ?>
    ...
    <form class="form-horizontal" method="post" action="" enctype="multipart/form-data">
        <fieldset>
            ...
            <div class="control-group <?echo $error_key;?>">
                ...
                <div class="controls">
                    <input type="text" id="inputk"  name="k" value="<?if(isset($_POST['k'])){echo $_POST['k'];}?>" class="input-xxlarge tblock" placeholder="Ключевые слова" maxlength="160" rel="5" />
...
<?}elseif(isset($_GET['add_pp'])){
    ...
    $prod=mysql_fetch_array(mysql_query("SELECT * FROM  `accessories` WHERE id='".$_GET['add_pp']."'"));
...
//если все впорядке Добавляем в базу
        ...
        if($ename=='' and  $elink=='' and $error_title=='' and $error_des=='' and $error_key=='' and $error_photo==''){
            ...
            $add=mysql_query("INSERT INTO `accessories_page` SET
        `name`='".$name."',
        `id_arr`='".$_GET['add_pp']."',
        `link`='".$link."',
        `k`='".$key."',
        ...
        `date`=NOW()");
            $id=mysql_insert_id();
            $add=mysql_query("UPDATE `accessories_page` SET `idd`='".$id."' WHERE `id`='".$id."'");
            include_once "../component/class.thumb.php";
            $ph_int2=count($photo_real);
            for($i=0;$i<$ph_int2;$i++){$foto_b = "../photo/volkswagen_img_".$id."_.jpg";$tn_image = new Thumbnail($photo_real[$i], 215); $tn_image->save($foto_b);}
            echo "<html> <head> <meta HTTP-EQUIV='Refresh' CONTENT='0; URL=./?catalog=".$_GET['catalog']."&arr=".$_GET['add_pp']."'></head><body>";
    ...
    <form class="form-horizontal" method="post" action="" enctype="multipart/form-data">
        <fieldset>
            ...
            <div class="control-group <?echo $error_title;?>">
                <label class="control-label" for="inputt">Title <i class="icon-question-sign po" style="margin: 7px 0px; 0 0" onclick="$('.bottom-left').notify({message: {text: 'Определяет заголовок документа.'}}).show();"></i></label>
                <div class="controls">
                    <input type="text" id="inputt"  name="t" value="<?echo $_POST['t'];?>" class="input-xxlarge tblock" placeholder="Title" maxlength="70" rel="4" />
                    <span id="span_text4"></span>
                </div>
            </div>
            ...
            <div class="control-group <?echo $error_key;?>">
                <label class="control-label" for="inputk">Ключевые слова <i class="icon-question-sign po" style="margin: 7px 0px; 0 0" onclick="$('.bottom-left').notify({message: {text: 'Набор ключевых слов близок к аннотации, указывать через запятую.'}}).show();"></i></label>
                <div class="controls">
                    <input type="text" id="inputk"  name="k" value="<?echo $_POST['k'];?>" class="input-xxlarge tblock" placeholder="Ключевые слова" maxlength="160" rel="5" />
                    <span id="span_text5"></span>
                </div>
            </div>
            <div class="control-group  <?echo $error_des;?>">
                <label class="control-label" for="inputd">description <i class="icon-question-sign po" style="margin: 7px 0px; 0 0" onclick="$('.bottom-left').notify({message: {text: 'Дается краткое описание содержания страницы.'}}).show();"></i></label>
                <div class="controls">
                    <input type="text" id="inputd" name="d" value="<?echo $_POST['d'];?>" class="input-xxlarge tblock" placeholder="description" maxlength="160" rel="6" />
                    <span id="span_text6"></span>
                </div>
            </div>
            ...
            <div class="control-group <?echo $error_photo;?>">
                ...
                <div class="controls">
                    <div class="fileupload fileupload-new" data-provides="fileupload">
                        ...
                        </div></div><big>Выполнить рассылку</big> <input name="rassilka" type="checkbox" value="1" /></div></div>
    ...
    <?if($_POST['rassilka']=='1'){
        ...
        while($cl = mysql_fetch_array($tq)){
            ...
            $to=$cl['mail'];
            $format=$cl['format'];
            $mail='news@karpaty-autocenter.com.ua';
            $hash=$cl['hash'];
            ...
            $text5.="Если Вы хотите отписаться от рассылки пройдите по ссылке:<br/>http://karpaty-autocenter.com.ua/page/register_user.php?hash=$hash \r\n";
...
<?}elseif(isset($_GET['edit_r'])){
    ...
    $prd=mysql_fetch_array(mysql_query("SELECT * FROM  `accessories_page` WHERE id='".$_GET['edit_r']."'"));

    $prod=mysql_fetch_array(mysql_query("SELECT * FROM  `accessories` WHERE id='".$prd['id_arr']."'"));
...
//если все впорядке Добавляем в базу
        ...
        if($_POST['name']!='' ){
            ...
            $add=mysql_query("UPDATE `accessories_page` SET  `name`='".$name."',
    `link`='".$link."',
    `k`='".$key."',
    `d`='".$des."',
    `t`='".$title."',
    ...
    WHERE `id`='".$_GET['edit_r']."'") or die(mysql_error());
            ...
            echo "<html> <head> <meta HTTP-EQUIV='Refresh' CONTENT='0; URL=./?catalog=".$_GET['catalog']."&arr=".$prd['id_arr']."'></head><body>";
        }}
    ...
    <form class="form-horizontal" method="post" action="" enctype="multipart/form-data">
        <fieldset>
            ...
            <div class="control-group <?echo $error_title;?>">
                <label class="control-label" for="inputt">Title <i class="icon-question-sign po" style="margin: 7px 0px; 0 0" onclick="$('.bottom-left').notify({message: {text: 'Определяет заголовок документа.'}}).show();"></i></label>
                <div class="controls">
                    <input type="text" id="inputt"  name="t" value="<?if(isset($_POST['t']) and $_POST['t']!=''){echo $_POST['t'];}else{echo $prd['t'];}?>" class="input-xxlarge tblock" placeholder="Title" maxlength="70" rel="4" />
                    <span id="span_text4"></span>
                </div>
            </div>
            ...
            <div class="control-group <?echo $error_key;?>">
                <label class="control-label" for="inputk">Ключевые слова <i class="icon-question-sign po" style="margin: 7px 0px; 0 0" onclick="$('.bottom-left').notify({message: {text: 'Набор ключевых слов близок к аннотации, указывать через запятую.'}}).show();"></i></label>
                <div class="controls">
                    <input type="text" id="inputk"  name="k" value="<?if(isset($_POST['k']) and $_POST['k']!=''){echo $_POST['k'];}else{echo $prd['k'];}?>" class="input-xxlarge tblock" placeholder="Ключевые слова" maxlength="160" rel="5" />
                    <span id="span_text5"></span>
                </div>
            </div>
            <div class="control-group  <?echo $error_des;?>">
                <label class="control-label" for="inputd">description <i class="icon-question-sign po" style="margin: 7px 0px; 0 0" onclick="$('.bottom-left').notify({message: {text: 'Дается краткое описание содержания страницы.'}}).show();"></i></label>
                <div class="controls">
                    <input type="text" id="inputd" name="d" value="<?if(isset($_POST['d']) and $_POST['d']!=''){echo $_POST['d'];}else{echo $prd['d'];}?>" class="input-xxlarge tblock" placeholder="description" maxlength="160" rel="6" />
                    <span id="span_text6"></span>
                </div>
            </div>
...
<?}elseif(isset($_GET['edit'])){
    ...
    $prod=mysql_fetch_array(mysql_query("SELECT * FROM  `accessories` WHERE id='".$_GET['edit']."'"));
...
//если все впорядке Добавляем в базу
        ...
        if($ename=='' and  $epname=='' and $elink=='' and $error_title=='' and $error_des=='' and $error_key==''){
            ...
            $key=($_POST['k']);
            $text=($_POST['text']);
            $add=mysql_query("UPDATE `accessories` SET `type`='".$_POST['type']."', `nmenu`='".$mname."',`link`='".$link."', `pmenu`='".$pmenu."', `k`='".$key."', `d`='".$des."', `t`='".$title."', `text`='".$text."'  WHERE `id`='".$_GET['edit']."'");
            if(in_array("1", $ph)){
                include_once "../component/class.thumb.php";
                $ph_int2=count($photo_real);
    ...
    <form class="form-horizontal" method="post" action="" enctype="multipart/form-data">
        <fieldset>
            ...
            <div class="control-group <?echo $error_title;?>">
                <label class="control-label" for="inputt">Title <i class="icon-question-sign po" style="margin: 7px 0px; 0 0" onclick="$('.bottom-left').notify({message: {text: 'Определяет заголовок документа.'}}).show();"></i></label>
                <div class="controls">
                    <input type="text" id="inputt"  name="t" value="<?if(isset($_POST['t'])){echo $_POST['t'];}else{echo $prod['t'];}?>" class="input-xxlarge tblock" placeholder="Title" maxlength="70" rel="4" />
                    <span id="span_text4"></span>
                </div>
            </div>
            ...
            <div class="control-group <?echo $error_key;?>">
                <label class="control-label" for="inputk">Ключевые слова <i class="icon-question-sign po" style="margin: 7px 0px; 0 0" onclick="$('.bottom-left').notify({message: {text: 'Набор ключевых слов близок к аннотации, указывать через запятую.'}}).show();"></i></label>
                <div class="controls">
                    <input type="text" id="inputk"  name="k" value="<?if(isset($_POST['k'])){echo $_POST['k'];}else{echo $prod['k'];}?>" class="input-xxlarge tblock" placeholder="Ключевые слова" maxlength="160" rel="5" />
                    <span id="span_text5"></span>
                </div>
            </div>
            <div class="control-group  <?echo $error_des;?>">
                <label class="control-label" for="inputd">description <i class="icon-question-sign po" style="margin: 7px 0px; 0 0" onclick="$('.bottom-left').notify({message: {text: 'Дается краткое описание содержания страницы.'}}).show();"></i></label>
                <div class="controls">
                    <input type="text" id="inputd" name="d" value="<?if(isset($_POST['d'])){echo $_POST['d'];}else{echo $prod['d'];}?>" class="input-xxlarge tblock" placeholder="description" maxlength="160" rel="6" />
                    <span id="span_text6"></span>
                </div>
            </div>
...
<?}else{
    ...
    if(isset($_GET['arr']) and $_GET['arr']!=''){
        ...
        */
        ...
        $prods=mysql_fetch_array(mysql_query("SELECT * FROM  `accessories` WHERE id='".$_GET['arr']."'"));



        if(isset($_GET['arov_down'])){

            $dow=mysql_query("SELECT * FROM  `accessories_page` WHERE idd>$_GET[arov_down] AND id_arr='".$prods['id']."'  ORDER BY idd ASC   LIMIT 1");
            $down=mysql_fetch_array($dow);
            $onm1=$down['idd'];
            $onm2=$_GET['arov_down'];
            $add1=mysql_query("UPDATE `accessories_page` SET idd='0'  WHERE idd='$onm1'");
            $add2=mysql_query("UPDATE `accessories_page` SET idd='$onm1'  WHERE idd='$onm2'");
            $add3=mysql_query("UPDATE `accessories_page` SET idd='$onm2'  WHERE idd='0'");
            echo "<html> <head> <meta HTTP-EQUIV='Refresh' CONTENT='0; URL=./?catalog=".$_GET['catalog']."&arr=".$_GET['arr']."'></head><body>";
        }
        if(isset($_GET['arov_up'])){
            $dow=mysql_query("SELECT * FROM  `accessories_page` WHERE idd<$_GET[arov_up] AND id_arr='".$prods['id']."' ORDER BY idd DESC  LIMIT 1");
            $down=mysql_fetch_array($dow);
            $onm1=$down['idd'];
            $onm2=$_GET['arov_up'];
            $add1=mysql_query("UPDATE `accessories_page` SET idd='0'  WHERE idd='$onm1'");
        ...
        if(isset($_GET['glass'])){
            ...
            $add1=mysql_query("UPDATE `accessories_page` SET glass='$sat'  WHERE id='$_GET[id]'");
            echo "<html> <head> <meta HTTP-EQUIV='Refresh' CONTENT='0; URL=./?catalog=".$_GET['catalog']."&arr=".$_GET['arr']."'></head><body>";
        }
        ...
        <div class="span3">
            <div class="well sidebar-nav">
                <ul class="nav nav-list">
                    ...
                    <li <?if(isset($_GET['add_pp'])){echo 'class="active"';}?> ><a href="?catalog=<?echo $_GET['catalog'];?>&add_pp=<?echo $_GET['arr'];?>"><i class="icon-plus"></i>Добавить страницу в радел</a></li>
                </ul>
            </div>
        </div>
        ...
        <div class="span9">
            ...
            <table class="table table-hover table-condensed table-bordered">
                ...
                <?
                $pr=mysql_query("SELECT * FROM  `accessories_page` WHERE id_arr='".$prods['id']."' ORDER BY idd DESC");
...
<td style="width: 115px;text-align: center;">';
                    ...
                    echo '<a href="?catalog='.$_GET['catalog'].'&edit_r='.$prod['id'].'"><i class="icon-pencil tool" data-original-title="Редактировать" ></i></a>
<i class="icon-remove tool delpp" rel="'.$prod['id'].'" data-original-title="Удалить"></i>
<a href="?catalog='.$_GET['catalog'].'&arr='.$prods['id'].'&glass='.$prod['glass'].'&id='.$prod['id'].'"><i class="'.$ic.' tool" data-original-title="Отключить"></i></a>
<a href="?catalog='.$_GET['catalog'].'&arr='.$prods['id'].'&arov_down='.$prod['idd'].'"><i class="icon-chevron-up tool" data-original-title="Поменять местами"></i></a>
<a href="?catalog='.$_GET['catalog'].'&arr='.$prods['id'].'&arov_up='.$prod['idd'].'"><i class="icon-chevron-down tool" data-original-title="Поменять местами"></i></a>
    ...
    <?}else{
        ...
        if(isset($_GET['arov_down'])){
            ...
            $dow=mysql_query("SELECT * FROM  `accessories` WHERE idd>$_GET[arov_down] AND menu='".$id_menu."'  ORDER BY idd ASC   LIMIT 1");
            $down=mysql_fetch_array($dow);
            $onm1=$down['idd'];
            $onm2=$_GET['arov_down'];
            $add1=mysql_query("UPDATE `accessories` SET idd='0'  WHERE idd='$onm1'");
            $add2=mysql_query("UPDATE `accessories` SET idd='$onm1'  WHERE idd='$onm2'");
            $add3=mysql_query("UPDATE `accessories` SET idd='$onm2'  WHERE idd='0'");
            echo "<html> <head> <meta HTTP-EQUIV='Refresh' CONTENT='0; URL=./?catalog=".$_GET['catalog']."'></head><body>";
        }
        if(isset($_GET['arov_up'])){
            $dow=mysql_query("SELECT * FROM  `accessories` WHERE idd<$_GET[arov_up] AND menu='".$id_menu."' ORDER BY idd DESC  LIMIT 1");
            $down=mysql_fetch_array($dow);
            $onm1=$down['idd'];
            $onm2=$_GET['arov_up'];
            $add1=mysql_query("UPDATE `accessories` SET idd='0'  WHERE idd='$onm1'");
        ...
        if(isset($_GET['glass'])){
            if($_GET['glass']=='0'){$sat=1;}else{$sat=0;}
            $add1=mysql_query("UPDATE `accessories` SET glass='$sat'  WHERE id='$_GET[id]'");
            echo "<html> <head> <meta HTTP-EQUIV='Refresh' CONTENT='0; URL=./?catalog=".$_GET['catalog']."'></head><body>";
        }
        ...
        <div class="span3">
            <div class="well sidebar-nav">
                <ul class="nav nav-list">
                    <li class="nav-header">Навигация</li>
                    <li <?if(isset($_GET['add_p'])){echo 'class="active"';}?> ><a href="?catalog=<?echo $_GET['catalog'];?>&add_p"><i class="icon-plus"></i>Добавить страницу</a></li>
                    ...
                    if (is_uploaded_file($_FILES['ban']['tmp_name'])) {
                        ...
                        if (filesize($filename) > $max_image_size) {
                            ...
                            if ($size[0] < $max_image_width	&& $size[1] < $max_image_height && $size[0] > $min_image_width	&& $size[1] > $min_image_height ) {
                                ...
                                if (move_uploaded_file($filename, "../img/".$_GET['catalog'].".jpg")) {
        ...
        <div class="span9">
            ...
            <table class="table table-hover table-condensed table-bordered">
                ...
                <tr>
                    <td colspan="3" >
                        <img src="../img/<?echo $_GET['catalog'];?>.jpg" />
                    </td>
                </tr>
                <?
                $pr=mysql_query("SELECT * FROM  `accessories` WHERE menu='".$id_menu."' ORDER BY idd ASC");
                while($prod=mysql_fetch_array($pr)){
                    echo '<tr class="';
...
<td style="width: 125px;text-align: center;">';
                    if($prod['type']==1){echo '<a href="?catalog='.$_GET['catalog'].'&add_pp='.$prod['id'].'"><i class=" icon-plus tool" data-original-title="Добавить страницу"></i></a>
<a href="?catalog='.$_GET['catalog'].'&arr='.$prod['id'].'"><i class="icon-share tool" data-original-title="Войти в раздел"></i></a>


';}

                    echo '<a href="?catalog='.$_GET['catalog'].'&edit='.$prod['id'].'"><i class="icon-pencil tool" data-original-title="Редактировать" ></i></a>
<i class="icon-remove tool del_p" rel="'.$prod['id'].'" data-original-title="Удалить"></i>
<a href="?catalog='.$_GET['catalog'].'&glass='.$prod['glass'].'&id='.$prod['id'].'"><i class="'.$ic.' tool" data-original-title="Отключить"></i></a>
<a href="?catalog='.$_GET['catalog'].'&arov_up='.$prod['idd'].'"><i class="icon-chevron-up tool" data-original-title="Поменять местами"></i></a>
<a href="?catalog='.$_GET['catalog'].'&arov_down='.$prod['idd'].'"><i class="icon-chevron-down tool" data-original-title="Поменять местами"></i></a>
