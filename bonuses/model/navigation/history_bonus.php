<?php
	...
	if (isset($_POST['id_user']) && $_POST['id_user'] !=='') {
		...
		if (count($sqlres)>1){
			...
			foreach($sqlres as $car) {
				$i++;
				echo '<a class="selectCar" data-car="'.$car['Model'].'" data-id="'.$i.'" href="?car='.$car['Model'].'">'.$car['Model'].' ('.$car['RegistrationNumber'].')</a> &nbsp;  &nbsp; ';
				$carModels[] = $car['Model'];
			}
			echo "</h5>";
		}
	...
	foreach($carModels as $car){
		...
		$j++;
		echo '<legend data-car="'.$car.'" class="data-cars id'.$j.'">История начисления баллов (Volkswagen '.$car.')</legend>';
			if (isset($_POST['id_user']) && $_POST['id_user'] !=='') {
				$CustomerCode=$db->my_mysql_select( 'SELECT сustomer_сode FROM user WHERE id_user="'.$_POST['id_user'].'"  ' );   
				//$sqlres=$db->my_mysql_select( 'SELECT * FROM documents WHERE CustomerCode="'.$CustomerCode['0']['сustomer_сode'].'"  GROUP BY `Zayavka`' );
				$sqlres=$db->my_mysql_select( 'SELECT DISTINCT * FROM documents WHERE CustomerCode="'.$CustomerCode['0']['сustomer_сode'].'" AND `Model` LIKE \''.$car.'\' GROUP BY `DocumentNumber`' );
			}

		foreach($sqlres as $zayavka){
			printDocs($zayavka, $CustomerCode['0']['сustomer_сode'], rand(1,1000), $j);
		}
		
		
	}
	
	function printDocs($zayavka, $userId, $rand, $j){
			require_once ($_SERVER['DOCUMENT_ROOT'].'/bonuses/model/system/config.php');
			
			require_once ($_SERVER['DOCUMENT_ROOT'].'/bonuses/model/system/config.php');
			$db=new mysql_conns ();			
			$sqlres=$db->my_mysql_select( 'SELECT `AmoutOfDiscount` FROM TableWork WHERE `DocumentNumber` = '.$zayavka['DocumentNumber'] );
			foreach($sqlres as $discounts){
				$discount += $discounts['AmoutOfDiscount'];
			}
			
			echo '<div style="margin-bottom:50px;" class="id'.$j.'">';
