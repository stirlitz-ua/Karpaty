...
<body>
<div align="center" style="height:250px; padding-bottom:100px;">
<!--img src="pic/banner.jpg" style="width:888px; height:345px;"--></a>
<iframe src="banner/index.html" style="margin-left:5px;width:889px; height:340px;" scrolling="no"></iframe>
</div>
...
<tr><td>
<input onblur="inputBG(this,0)"  onfocus="inputBG(this,1)" name="poisk" type="text" placeholder="Введите артикул или название аксесуара..." style="width:239px; margin-left:-25px;" />
<input name="act" type="hidden" value="poisk"/></td><td>
<input style="margin-left:-658px;height:25px; width:60px; cursor:pointer;" type="submit" value="Поиск" />
</td></tr>
</table>
</form>
</div>
...
<!-- ???????? ??????? -->
<table width="884" border="0" cellspacing="3" cellpadding="0" align="center" style="margin-top:15px; border:1px solid #eee;">
  <tr>
    <td valign="top" width="260">
  
<?
if($_GET['act']==''){
echo'<h1 style="margin-left:-50px;">Универсальные аксесуары:</h1>';
...
if (mysql_num_rows($basecat) > 0)
{
	...
	$basecat1 = mysql_query("SELECT `cat_id`,`cat_name`,`glass` FROM `cat_cat` WHERE `cat_papa`='114' AND `glass` is NULL ORDER BY `cat_name`",$db) or die(mysql_error());
		...
		echo '<div id="catalog">
                            <a href="http://karpaty-autocenter.com.ua/catalog/katalogi_aksessuarov/" target="_blank">
                                <img class="header" src="http://karpaty-autocenter.com.ua/accessories/images/catalog-header.jpg">
                                <img class="image" src="http://karpaty-autocenter.com.ua/accessories/images/catalog-image.jpg">
                            </a>
                        </div></dl>';
}
	}
...
echo'<h1 style="margin-left:-140px;">Модели</h1>';
$basecat = mysql_query("SELECT `cat_id`,`cat_name`,`cat_pic` FROM `cat_cat` WHERE `cat_papa`='0' ORDER BY `cat_name`",$db) or die(mysql_error());
if (mysql_num_rows($basecat) > 0)
{
	...
	if (isset($_COOKIE['Podcat'.$rowcat['cat_id']]))	{ $nev=''; $otkr = 1; $src = 'pic/cat-dt.gif'; } 
...
$basecat = mysql_query("SELECT `cat_id`,`idd`,`cat_name`,`cat_pic`,`glass` FROM `cat_cat` WHERE `cat_papa`='0' AND `glass` is NULL ORDER BY `idd` ASC",$db) or die(mysql_error());
if (mysql_num_rows($basecat) > 0)
{
	$rowcat = mysql_fetch_array($basecat);
	do { 
	...
	<a href="cat.php?act=cat&cat_id='.$rowcat['cat_id'].'"><img cat="'.$rowcat['cat_id'].'"  src="pic/'.$rowcat['cat_pic'].'" style="float:left; margin: 4px 5px 0 0; cursor:pointer;" />
'.$rowcat['cat_name'].'</a><span style="margin-left:3px;" class="sm2">'.$kolvotov.'</span></div>';				
					$kolvotov = '';
	
	$basecat1 = mysql_query("SELECT `cat_id`,`cat_name`,`glass` FROM `cat_cat` WHERE `cat_papa`='{$rowcat['cat_id']}' AND `glass` is NULL  ORDER BY `cat_name`",$db) or die(mysql_error());
...
if (isset($cat_id)) 
	...
	if (isset($tov_id))
	{
		...
		<img style="margin:0 10px 3px 0;" width="215" height="130" src="pic/tov/sm_'.$rowtov['tov_foto'].'"></a></td>'; } 
		else { $tov_foto = '<td></td>'; }
		$tov_nazv = $rowtov['tov_nazv'];
	...
	if (mysql_num_rows($basefoto) > 0)
		...
		$rowfoto = mysql_fetch_array($basefoto);
			...
			<img style="margin-right:215px;" height="130" src="pic/tov/sm_'.$rowfoto['foto_file'].'"></a></td></tr></table>';
			}
			while ($rowfoto = mysql_fetch_array($basefoto));
		echo '</td></tr>';
		}
		
		echo '<tr><td colspan="2"><div id="description"><div class="border"><h2>Описание</h2>'.$tov_opis.'</div></div></td></tr>';
		echo '</table>';
		if ($rowtov['tov_com']==1) {	$com='c'.$rowtov['tov_id']; include("com.php"); echo '<br /><br />';}
	}	
	else {
$basecat = mysql_query("SELECT `cat_id`,`cat_name`,`cat_pic` FROM `cat_cat` WHERE `cat_papa`='{$cat_id}' ORDER BY `cat_name`",$db)
or die(mysql_error());
		if (mysql_num_rows($basecat) > 0)
		{
			$rowcat = mysql_fetch_array($basecat);
			echo '<div id="menu-ico"><dl class="cat" style="width:450px;">';
			do { 
				...
				echo '<div class="ico"><a href="cat.php?act=cat&cat_id='.$rowcat['cat_id'].'" 
					title="'.$rowcat['cat_name'].'"><img src="pic/'.$rowcat['cat_pic'].'">'.$rowcat['cat_name'].'</a>
					<span style="margin-left:3px;" class="sm2">'.$kolvotov.'</span></div>';				
					$kolvotov = '';
			}
			while($rowcat = mysql_fetch_array($basecat));
			echo '</dl></div>';
...
$basetov = mysql_query("SELECT * FROM `cat_tov` WHERE `tov_cat`='{$cat_id}' ORDER BY `tov_id` DESC LIMIT $start, $num",$db) or die(mysql_error());
				...
				echo '<table cellspacing="0" cellpadding="0" style="margin:0 0 15px 0; border:1px solid #f2f2f2;"><tr>';
					do { 
						...
						if ($cv == 1) { $bgzapis = "#ffffff"; $cv=0; } else { $bgzapis = "#f5f5f5"; $cv++; }
						if (!empty($rowtov['tov_foto'])) { 
						$img = '<a href="cat.php?mod=cat&cat_id='.$cat_id.'&tov_id='.$rowtov['tov_id'].'"><img align="center" width="215" height="130" style="margin:10px 0 10px 0;" src="pic/tov/sm_'.$rowtov['tov_foto'].'" border="0" /><br /></a>';  }
						else { $img="<br /><br /><br /><br />"; }
						if (!empty($rowtov['tov_starcena'])) { 
						$tov_starcena = '<p><s>Старая цена: '.$rowtov['tov_starcena'].' '.$cat_val.'</s></p>';  }
						else { $tov_starcena=""; }
...
// Вывод меню если страниц больше одной
  ...
  echo '</div><br />';
				...
				{
				echo "<br />В данной категории пока нет аксессуаров<br /><br /><br /><br />"; 	
				}
			}
	}
?>
...
if (isset($_GET['page'])) {$page = $_GET['page']; }
if (isset($_POST['page'])) {$page = $_POST['page']; }
$base1 = mysql_query("SELECT COUNT(*) FROM `cat_tov` WHERE (`tov_nazv` LIKE '%$poisk%') OR (`tov_artic`  LIKE '%$poisk%') OR (`tov_opis` LIKE '%$poisk%')");
$temp = mysql_fetch_array($base1);
$posts = $temp[0];
$total = (($posts - 1) / $num) + 1;
$total =  intval($total);
...
if($page + 2 <= $total) $page2right = ' &nbsp;<a class="nav" href='.$link.'&page='.($page + 2).'>'.($page + 2).'</a>&nbsp; ';
if($page + 1 <= $total) $page1right = ' &nbsp;<a class="nav" href='.$link.'&page='.($page + 1).'>'.($page + 1).'</a>&nbsp; ';
$baseob = mysql_query("SELECT * FROM `cat_tov` WHERE (`tov_nazv` LIKE '%$poisk%') OR (`tov_artic`  LIKE '%$poisk%') OR (`tov_opis` LIKE '%$poisk%') LIMIT $start, $num",$db) or die(mysql_error()); 
if (empty($poisk)) { echo '<p class="alert">Обязательно введите ключевое слово для поиска</p>';}
			...
			echo '</div>';
				...
				$img = '<td valign="top" style="margin:0; padding:0;"><img width="215px" height="130" align="left" style="margin:5px 10px 0 0;" 
		  	...
		  	$basekuda = mysql_query("SELECT `cat_name` FROM `cat_cat` 
			WHERE `cat_id`='{$rowob['tov_cat']}' LIMIT 1",$db) or die(mysql_error());
			$kuda = mysql_fetch_array($basekuda);
